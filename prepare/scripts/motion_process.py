...
import torch
from prepare.paramUtil import *
from prepare.common.quaternion import *
from prepare.common.skeleton import Skeleton
...
def recover_rot_pos_velocity(data):
    """
    Same as recover_rot but include in the final features also the rifke positions and xyz velocities (6+3+3 features for each joint)
    """
    bs, seq_len, dims = data.shape
    # dataset [bs, seqlen, 263/251] HumanML/KIT
    joints_num = 22 if data.shape[-1] == 263 else 21
    ...
    rot_pos = torch.cat([cont6d_params, pos_params, velocity_params], dim=-1)    # bs x seqlen x num_joints x 9
    return rot_pos
